...
def FormatBackendUrl(Endpoint: str) -> str:
    """
    Formats the given endpoint into a full URL to the SUBTER backend server.
    """
...
async def BuildUserEmbed( UserObject: dict ) -> discord.Embed:
    ...
    UserEmbed.url = f"https://www.subter.org/users/{UserObject['id']}/profile"
    UserEmbed.set_thumbnail(url=f"https://www.subter.org/Thumbs/Head.ashx?x=100&y=100&userId={UserObject['id']}")
...
async def BuildItemEmbed( ItemObject: dict ) -> discord.Embed:
    ...
    ItemEmbed.url = f"https://www.subter.org/catalog/{ItemObject['id']}/"
    ItemEmbed.set_thumbnail(url=f"https://www.subter.org/Thumbs/Asset.ashx?x=180&y=180&assetId={ItemObject['id']}")
    ...
    if ItemObject["creator"] != None:
        ItemEmbed.add_field(name="Creator", value=f"[{ItemObject['creator']['username']}](https://www.subter.org/users/{ItemObject['creator']['id']}/profile)")
    else:
...
@DiscordClient.tree.command(name="ping", description="Pings the SUBTER website, and returns the response time.")
async def ping( interaction: discord.Interaction ):
    """
    Pings the SUBTER backend server, and returns the response time.
    """
    ...
    try:
        ...
        await interaction.response.send_message(embed = discord.Embed(
            title="Error! :(",
            description=f"An error occured while trying to ping the SUBTER website maybe it's down?",
    ...
    await interaction.response.send_message(embed = discord.Embed(
         title="Pong!",
         description=f"SUBTER website responded in `{round(time.time() - StartTime, 3)}s`",
         color=discord.Color.green()
    ))

@DiscordClient.tree.command(name="lookup-username", description="Searches SUBTER for a user with the given username.")
@app_commands.describe(username="Searches SUBTER for a user with the given username.")
async def lookup_username( interaction: discord.Interaction, username: str ):
    """
    Requests info from the SUBTER website about a user with the given username.
    """
    RequestSession = await GetRequestSession()
    try:
        ...
        await interaction.response.send_message(embed = discord.Embed(
            title="Error! :(",
            description=f"An error occured while trying to lookup the username `{username}` maybe the SUBTER website is down?",
            color=discord.Color.red()
        ))
        return
...
@DiscordClient.tree.command(name="lookup-userid", description="Searches SUBTER for a user with the given userId.")
@app_commands.describe(userid="Searches SUBTER for a user with the given userId.")
async def lookup_userid( interaction: discord.Interaction, userid: int ):
    """
    Requests info from the SUBTER website about a user with the given userId.
    """
    RequestSession = await GetRequestSession()
    try:
        ...
        await interaction.response.send_message(embed = discord.Embed(
            title="Error! :(",
            description=f"An error occured while trying to lookup the userId `{userid}` maybe the SUBTER website is down?",
            color=discord.Color.red()
        ))
        return
...
@DiscordClient.tree.command(name="lookup-user", description="Searches SUBTER for a user with the given member.")
@app_commands.describe(member="Searches SUBTER for a user with the given member.")
async def lookup_user( interaction: discord.Interaction, member: discord.Member ):
    """
    Requests info from the SUBTER website about a user with the given member.
    """
    RequestSession = await GetRequestSession()
    try:
        ...
        await interaction.response.send_message(embed = discord.Embed(
            title="Error! :(",
            description=f"An error occured while trying to lookup the account for <@{member.id}> maybe the SUBTER website is down?",
            color=discord.Color.red()
        ))
        return
...
@DiscordClient.tree.command(name="lookup-item", description="Searches SUBTER for an item with the given itemid.")
@app_commands.describe(itemid="Searches SUBTER for an item with the given itemid.")
async def lookup_item( interaction: discord.Interaction, itemid: int ):
    ...
    try:
        ...
        await interaction.response.send_message(embed = discord.Embed(
            title="Error! :(",
            description=f"An error occured while trying to lookup for Item `{itemid}` maybe the SUBTER website is down?",
            color=discord.Color.red()
        ))
        return
...
@DiscordClient.tree.command(name="claim-roles", description="Claims your roles from the SUBTER website.")
async def claim_roles( interaction: discord.Interaction ):
    ...
    try:
        ...
        await interaction.response.send_message(embed = discord.Embed(
            title="Error! :(",
            description=f"An error occured while trying to retrieve info about you, maybe the SUBTER website is down?",
            color=discord.Color.red()
        ))
        return
...
@DiscordClient.tree.command(name="claim-turbo", description="Claim your Turbo Builders Club after boosting the server. ( Must have a linked SUBTER Account )")
async def claim_turbo( interaction: discord.Interaction ):
    ...
    try:
        ...
        await interaction.response.send_message(embed = discord.Embed(
            title="Error! :(",
            description=f"An error occured while trying to contact the SUBTER website, maybe SUBTER is down?",
            color=discord.Color.red()
        ))
        return
